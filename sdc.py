...
# Initialize counters for each direction's images to label them uniquely
fnum=0
snum=0
bnum=0
rnum=0
...
class window(QtWidgets.QMainWindow):
   """
   Main GUI Window for controlling the car and collecting data.
   From here you can connect to the Arduino (via Serial) and the IP Webcam.
   """
   ...
   def openport(self):
      """
      Open Serial connection to the Arduino and set camera URL.
      """
      com=self.comport.text()
      self.url=self.cameraurl.text()
      # Open serial port at 9600 baud rate (Make sure this matches Arduino code)
   ...
   def forward(self):
      """
      Move car Forward, capture image, and save with label 'forward'.
      """
      global fnum
      # Send 'f' command to Arduino via Serial
      self.s.write(b'f')
      # Capture image from the IP Webcam URL
      img=io.imread(self.url)
      # Save image to 'testsdc' directory (Ensure this path is correct/updated)
   ...
   def right(self):
      """
      Turn car Right, capture image, and save with label 'right'.
      """
      global rnum
      self.s.write(b'r')
      img=io.imread(self.url)
   ...
   def left(self):
      """
      Turn car Left, capture image, and save with label 'left'.
      """
      global lnum
      self.s.write(b'l')
      img=io.imread(self.url)
   ...
   def backward(self):
      """
      Move car Backward, capture image, and save with label 'backward'.
      """
      global bnum
      self.s.write(b'b')
      img=io.imread(self.url)
   ...
   def stopdir(self):
      """
      Stop the car, capture image, and save with label 'stop'.
      """
      global snum
      self.s.write(b's')
      img=io.imread(self.url)
